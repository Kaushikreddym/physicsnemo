#!/bin/bash
#SBATCH --job-name=generate_mswxdwd             # Job name
#SBATCH --output=submit/logs/generate_mswxdwd.%j.out  # STDOUT file
#SBATCH --error=submit/logs/generate_mswxdwd.%j.err   # STDERR file
...
if [ -z "$RES_CKPT" ]; then
    ...
    echo "Please train the diffusion model first using diffusion_mswxdwd.sh"
    exit 1
fi
...
for YEAR in "${ALL_YEARS[@]}"; do
    ...
    OUTPUT_FILE="./generated/mswxdwd_corrdiff_fulldomain_${YEAR}.nc"
